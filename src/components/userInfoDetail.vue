...
<div class="app" v-cloak @click="appActive">
            ...
            <div class="frame">
                ...
                <div class="listArea">
                    <ul>
                        ...
                        <li>
                            <span>真实姓名</span>
                            <span>{{userInfoData.RealName}}</span>
                        </li>
                        <li>
                            <span>角色</span>
                            <span>{{roleName}}</span>
                        ...
                        <li>
                            <span>条码</span>
                            <span>{{userInfoData.BarCode}}</span>
                        </li>
                        <li>
                            <span>状态</span>
                            <span>{{userInfoData.IsEnabled == 1 ? "可用" : "不可用"}}</span>
                        </li>
                        <li>
                            <span>手机</span>
                        ...
                        <li>
                            <span>QQ</span>
                            <span>{{userInfoData.QQ}}</span>
                        </li>
                        <li>
                            <span>性别</span>
                            <span>{{userInfoData.Sex != "1" ? "女" : "男"}}</span>
                        </li>
                        <li>
                            <span>生日</span>
                            <span>{{userInfoData.Birthday ? userInfoData.Birthday.replace("T", " ") : ""}}</span>
                        </li>
                    </ul>
                </div>
    ...
    data:function(){
        ...
        computed: {
            ...
            reagentDetailDataStatus: function () {
                if (this.reagentDetailData.Status == 1) {
                    return "在库";
                }
                else if (this.reagentDetailData.Status == 2) {
                    return "出库";
                }
                else if (this.reagentDetailData.Status == 3) {
                    return "空瓶";
                }
                else {
                    return "数据错误";
        ...
        methods: {                filterNoWarningToday: function () {                    Cab.FilterNoWarningToday();                },
            ...
            //用户自定义方法————————————————————————————————————————————————————————————————————————————————————————————————
            updateUserInfoData: function () {
                try {
                    var jsonStr = User.MyUserInfo();
                    this.userInfoData = JSON.parse(jsonStr);
                    this.roleName = User.GetRoleName(this.userInfoData.RoleId);
                }
                catch (e) {
                    var that = this;
                    this.popupAlert("数据错误", "无法获得当前登录用户的详细信息！", 3000, function () {
                        that.pageBack();
                    }, function () {
                        that.pageBack();
                    });
