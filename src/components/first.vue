  ...
  <div class="content">
    <div class="body">
      <div class="frame" style="position: relative;">
        ...
        <div class="rows info-display" style="width:390px;height:230px;position:absolute;left:20px">
          <div class="box-tilte">本次累计处理废液量(L)</div>
          <div class="box-content1">{{fyPumpTotalCapValue}}</div>

        </div>
        <div class="rows info-display" style="width:200px;height:110px;position:absolute;left:420px;background: rgba(15, 50, 50, 0.8);">
          <div class="box-tilte">氢离子浓度指数(pH)</div>
          <div class="box-content2">{{phValue}}</div>
        </div>
        <div class="rows info-display" style="width:200px;height:110px;position:absolute;left:420px;top:120px;background: rgba(15, 50, 50, 0.8);">
          <div class="box-tilte">实时温度(°C)</div>
          <div class="box-content2">{{tempValue}}</div>
        </div>
        <div class="rows info-display" style="width:150px;height:240px;position:absolute;left:630px;background: rgba(15, 50, 50, 0.8);">
          <div class="box-tilte">搅拌速度(0~10)</div>
        ...
        <div class="mainButtonBox">
          <div style="position:absolute;top:320px;left:6px">
            <button v-show='isStart==true' class="button button--ujarak button--border-thin button--inverted button--text-thick mainButton" id="stopButton" @click="stopRun()">停止</button>
            <button v-show='isStart==false' class="button button--ujarak button--border-thin button--inverted button--text-thick mainButton" @click="btnBackClickEvent">返回</button>
            <button class="button button--ujarak button--border-thin button--inverted button--text-thick mainButton" id='qingxiButton'>清洗</button>
            <button class="button button--ujarak button--border-thin button--inverted button--text-thick mainButton" id='paiyeButton'>手动排液</button>
          </div>
          <div style="position:absolute;top:320px;right:8px">
            <button v-show='isStart==false' class="button button--ujarak button--border-thin button--inverted button--text-thick mainButton mainHover" id="startButton" @click="startRun()">开始</button>
          </div>
        </div>
      </div>
...
<script>
  export default {
    ...
    data: function () {
      return {
        cabNum: -1,
        pageTitle: this.$parent.processType==1? '酸性废液处理程序':'碱性废液处理程序',
        intervalId: null,
        startUpTime: 0,
        TotalRunTime:0,
        isStart:false,
    ...
    methods: {
      ...
      speedSet:function(value){
        sys.setStirSpeed(value);
      },
    },
    mounted: function () {
