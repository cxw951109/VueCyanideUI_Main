...
(function (factory) {
  ...
  $.fn.extend({
    'virtualkeyboard': function (options) {
      ...
      let keyboardHTML = ''
      keyboardHTML += '<div class="virtualkeyboard" style="display:none">'
      ...
      keyboardHTML += '<span class="slideDown">↓</span>'
      keyboardHTML += '</div>'
      ...
      $(this).click((event) => {
        if (!['INPUT'].includes(event.target.tagName) || !['text','password','number'].includes(event.target.type)) return
      ...
      // 绑定点击事件，在点击键盘区域之外时隐藏键盘
        .click((e) => {
          const el = $(e.target)
          if (!el.parents('.virtualkeyboard').length && !el.hasClass('word') && !el.hasClass('slideDown')) {
            clearZhWords()
            $('.virtualkeyboard').hide()
          }
        })
