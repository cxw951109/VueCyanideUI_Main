...
var SimpleInputMethod =
    {
         ...
         */
        initDom: function () {
            var temp = '<div class="pinyin"></div><div class="result"><ol></ol><div class="page-up-down"><span class="page-up">上页</span>&nbsp;<span class="page-down">&nbsp;下页</span></div></div>';
        ...
        init: function (selector) {
            ...
            var that = this;
            this.currentVal = $(".ui-keyboard-preview-wrapper .ui-keyboard-preview").eq(0).val();
            this.xs = "";

            var bs = false;
            $(".ui-keyboard-button").bind("click touchend", function () {

                var zhi = $(this).attr("data-value");

                if (zhi == "bksp") {

                    that.delChar();

                    that.currentVal = $(".ui-keyboard-preview-wrapper .ui-keyboard-preview").eq(0).val();
                    // preventDefault = true;
                }
                else if (zhi == "accept") { }
                else if (zhi == "中") {

                    $(this).attr("data-action", "英");
                    $(this).attr("data-value", "英");
                    $(this).attr("data-name", "英");
                    $(this).attr("data-html", '<span class="ui - keyboard - text">英</span>');
                    $(this).find(".ui-keyboard-text").text("英");
                    $(this).addClass("ui-state-active");
                    bs = true;
                    $(".ui-keyboard-preview-wrapper .ui-keyboard-preview").eq(0).val(that.currentVal);

                   
                }
                else if (zhi == "英") {
                    $(this).attr("data-action", "中");
                    $(this).attr("data-value", "中");
                    $(this).attr("data-name", "中");
                    $(this).attr("data-html", '<span class="ui - keyboard - text">中</span>');
                    $(this).find(".ui-keyboard-text").text("中");
                    ...
                    if (bs == true) {
                        ...
                        $(".ui-keyboard-preview-wrapper .ui-keyboard-preview").eq(0).val(that.currentVal);
                        that.xs += $(this).attr("data-value");
                        //alert($(".ui-keyboard-preview-wrapper .numkeyboard").eq(0).val());
                        that.addChar(that.xs, $(this).parent().parent().find(".ui-keyboard-preview").eq(0)[0]);
                        // preventDefault = true;
            ...
            for (var i = 0; i < obj.length; i++) {
                obj[i].addEventListener('input', function (e) {
        ...
        selectHanzi: function (i) {
            ...
            {
                this.xs = "";
                this.currentVal += this.hanzi;
                this._input.value += this.hanzi;
                var t= $(".ui-keyboard-preview-wrapper .ui-keyboard-preview").eq(0).val();
                $(".ui-keyboard-preview-wrapper .ui-keyboard-preview").eq(0).val("").focus().val(t);
                this.hide();
            }
        },
        /**
        ...
        show: function (obj) {
            ...
            this._target.style.top = pos.top - 150 + document.body.scrollTop + 'px';
            this._input = obj;
